...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=PORT, debug=False)            os.environ.get(f"ELEVENLABS_API_KEY_{index}", "").strip()
            or os.environ.get(f"ELEVENLABS_KEY_{index}", "").strip()
            or os.environ.get(f"ELAVENLABS_API_KEY_{index}", "").strip()
        )
        if value:
    ...
    if values:
        return values
    return [x.strip() for x in os.environ.get("ELEVENLABS_API_KEYS", "").split(",") if x.strip()]
...
def elevenlabs(method: str, url: str, body: dict[str, Any] | None = None, audio: bool = False):
    available = get_keys()
    if not available:
        raise RuntimeError("No API key found. Add ELEVENLABS_API_KEY_1 in Space Secrets.")
    last_error = "ElevenLabs request failed."
    for attempt in range(len(available)):
        with lock:
            key_index = (state["active"] + attempt) % len(available)
        try:
            ...
            if response.status_code not in {401, 402, 429, 500, 502, 503, 504}:
                break
            log.warning("ElevenLabs key %s failed HTTP %s, rotating", key_index + 1, response.status_code)
        except requests.RequestException as exc:
            last_error = str(exc)
            log.warning("ElevenLabs key %s network error, rotating", key_index + 1)
    with lock:
        state["errors"] += 1
    raise RuntimeError(last_error)
    ...
    for endpoint in (
        ...
        try:
            ...
            if voices:
                voice_cache.update({"voices": voices, "expires": now + 300})
                return voices
            failures.append(f"{endpoint}: empty voice list")
        except Exception as exc:
            failures.append(f"{endpoint}: {exc}")
    raise RuntimeError("Voice loading failed. " + " | ".join(failures))
    ...
    result = [
        {
            ...
            "category": voice.get("category", "account"),
...
def synthesis_text(script: str, tags: list[str], rules: str, speed: float) -> str:
    clean_tags = [str(tag).strip() for tag in (tags or []) if str(tag).strip()]
    prefix = (
        f"Delivery directions: {', '.join(clean_tags)}. Apply naturally and strictly.\n\n"
        if clean_tags else ""
    )
    speed_value = float(speed or 1.0)
    pace = (
...
def synthesize(voice_id: str, text: str, stability: float = 0.8, clarity: float = 0.6, speed: float = 1.0):
    body = {
        "text": text,
        "model_id": MODEL_ID,
        "voice_settings": {
...
def voices():
    try:
        ...
        return jsonify({"source": "account_with_legacy_fallback", "voices": catalog, "count": len(catalog)})
    except Exception as exc:
        return jsonify({"error": str(exc), "hint": "Replace key and rebuild."}), 503
...
def preview():
    ...
    voice_id = str(payload.get("voice_id", "")).strip()
    if not voice_id:
        return jsonify({"error": "Select a voice first."}), 400
    try:
        audio, _ = synthesize(voice_id, "This is a short AuraVoice Studio preview.")
        return jsonify({"audio_base64": base64.b64encode(audio).decode("ascii"), "mime_type": "audio/mpeg"})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 503
...
def generate():
    ...
    try:
        ...
        text = synthesis_text(script, payload.get("tags", []), payload.get("pronunciation", ""), speed)
        audio, key_index = synthesize(
            voice_id, text,
            payload.get("stability", 0.8),
            payload.get("similarity_boost", 0.6),
        ...
        return jsonify({
            "audio_base64": base64.b64encode(audio).decode("ascii"),
            "mime_type": "audio/mpeg",
            "key_label": f"Key {key_index + 1:02d}",
        })
    except Exception as exc:
        return jsonify({"error": str(exc)}), 503
...
def convert_audio(mp3: bytes, output_format: str) -> io.BytesIO:
    ...
    args = codec_map.get(output_format)
    if not args:
        return io.BytesIO(mp3)
    with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as src:
        src.write(mp3)
        src_path = src.name
    try:
        process = subprocess.run(
            ["ffmpeg", "-y", "-i", src_path, *args, "pipe:1"],
...
def export_audio():
    try:
        ...
        output_format = str(payload.get("format", "mp3")).lower()
        raw = base64.b64decode(str(payload.get("audio_base64", "")))
        output = io.BytesIO(raw) if output_format == "mp3" else convert_audio(raw, output_format)
        output.seek(0)
        mime_map = {
        ...
        return send_file(output, mimetype=mime, as_attachment=True, download_name=f"auravoice.{output_format}")
    except Exception as exc:
